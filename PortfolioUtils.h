...
// Compute PV01Parallel (i.e. sensitivity with respect to parallel shift of the yield curve (all risk factor move simultaneously))
// Use central differences, absolute bump of 0.01%, rescale result for rate movement of 0.01%
std::vector<std::pair<string, portfolio_values_t>> compute_pv01_parallel(const std::vector<ppricer_t>& pricers, const Market& mkt);

// Compute PV01Bucketed (i.e. sensitivity with respect to individual yield curves (the yield curve for each tenor Ti change, with all the rest remaining constant))
// Use central differences, absolute bump of 0.01%, rescale result for rate movement of 0.01%
